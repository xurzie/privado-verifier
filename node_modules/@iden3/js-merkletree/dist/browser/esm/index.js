...
const NODE_TYPE_MIDDLE = 0;
// Leaf node.ts with a key and a value
const NODE_TYPE_LEAF = 1;
// empty node.ts
const NODE_TYPE_EMPTY = 2;
const NODE_VALUE_BYTE_ARR_LENGTH = 65;
const EMPTY_NODE_VALUE = new Uint8Array(NODE_VALUE_BYTE_ARR_LENGTH);
...
const PROOF_FLAG_LEN = 2;
const NOT_EMPTIES_LEN = ELEM_BYTES_LEN - PROOF_FLAG_LEN;
...
const MAX_NUM_IN_FIELD = FIELD_SIZE - BigInt('1');
...
class Data {
    ...
    equal(d2) {
        return (bytesEqual(this._value[0].value, d2.value[0].value) &&
...
const ErrNodeKeyAlreadyExists = 'key already exists';
// ErrKeyNotFound is used when a key is not found in the MerkleTree.
const ErrKeyNotFound = 'Key not found in the MerkleTree';
// ErrNodeBytesBadSize is used when the data of a node has an incorrect
// size and can't be parsed.
const ErrNodeBytesBadSize = 'node data has incorrect size in the DB';
// ErrReachedMaxLevel is used when a traversal of the MT reaches the
// maximum level.
const ErrReachedMaxLevel = 'reached maximum level of the merkle tree';
// ErrInvalidNodeFound is used when an invalid node is found and can't
// be parsed.
const ErrInvalidNodeFound = 'found an invalid node in the DB';
// ErrInvalidProofBytes is used when a serialized proof is invalid.
const ErrInvalidProofBytes = 'the serialized proof is invalid';
...
const ErrInvalidDBValue = 'the value in the DB is invalid';
// ErrEntryIndexAlreadyExists is used when the entry index already
// exists in the tree.
const ErrEntryIndexAlreadyExists = 'the entry index already exists in the tree';
// ErrNotWritable is used when the MerkleTree is not writable and a
// write function is called
const ErrNotWritable = 'Merkle Tree not writable';

const ErrNotFound = 'key not found';
...
const ErrNodeAuxNonExistAgainstHIndex = 'non-existence proof being checked against hIndex equal to nodeAux';
...
export { CircomProcessorProof, CircomVerifierProof, DATA_LEN, DATA_LEN_BYTES, Data, ELEM_BYTES_LEN, EMPTY_NODE_STRING, EMPTY_NODE_VALUE, ElemBytes, Entry, ErrEntryIndexAlreadyExists, ErrInvalidDBValue, ErrInvalidNodeFound, ErrInvalidProofBytes, ErrKeyNotFound, ErrNodeBytesBadSize, ErrNodeKeyAlreadyExists, ErrNotFound, ErrNotWritable, ErrReachedMaxLevel, FIELD_SIZE, HASH_BYTES_LENGTH, Hash, InMemoryDB, IndexedDBStorage, LocalStorageDB, MAX_NUM_IN_FIELD, Merkletree, NODE_TYPE_EMPTY, NODE_TYPE_LEAF, NODE_TYPE_MIDDLE, NODE_VALUE_BYTE_ARR_LENGTH, NOT_EMPTIES_LEN, NodeEmpty, NodeLeaf, NodeMiddle, PROOF_FLAG_LEN, Proof, ZERO_HASH, bigIntToUINT8Array, bigint2Array, bytes2BinaryString, bytes2Hex, bytesEqual, checkBigIntInField, checkEntryInField, circomSiblingsFromSiblings, elemBytesToBigInts, getPath, hashElems, hashElemsKey, newBigIntFromBytes, newDataFromBytes, newHashFromBigInt, newHashFromHex, newHashFromString, rootFromProof, setBitBigEndian, siblignsFroomProof, siblings2Bytes, str2Bytes, swapEndianness, testBit, testBitBigEndian, verifyProof };
