...
__export(index_exports, {
  MerklizationConstants: () => MerklizationConstants,
  Merklizer: () => Merklizer,
  MtValue: () => MtValue,
  Path: () => Path,
  PoseidonHasher: () => PoseidonHasher,
  Temporal: () => import_polyfill4.Temporal,
...
var PoseidonHasher = class {
  constructor(_hasher = import_js_crypto.poseidon) {
...
var validateValue = (val) => {
  switch (typeof val) {
    ...
    case "number":
      return;
    case "object":
      if (val instanceof import_polyfill.Temporal.Instant) {
        return;
      }
  }
  throw new Error(
  ...
  switch (dataType) {
    ...
    case "http://www.w3.org/2001/XMLSchema#dateTime" /* DateTime */: {
      ...
      if (dateRegEx.test(valueStr)) {
        return import_polyfill.Temporal.Instant.from(new Date(valueStr).toISOString());
      }
      return import_polyfill.Temporal.Instant.from(valueStr);
    }
...
var _MtValue = class _MtValue {
  ...
  isTime() {
    return this.value instanceof import_polyfill2.Temporal.Instant;
  }
  asTime() {
    if (!this.isTime()) {
      throw MerklizationConstants.ERRORS.MT_VALUE_INCORRECT_TYPE;
...
__publicField(_MtValue, "mkValueMtEntry", (h, v) => {
  switch (typeof v) {
    case "number":
    ...
    default: {
      if (v instanceof import_polyfill2.Temporal.Instant) {
...
__publicField(_MtValue, "mkValueUInt", (h, v) => {
  return BigInt.asUintN(64, v);
});
__publicField(_MtValue, "mkValueBool", (h, v) => {
  if (v) {
    return h.hash([BigInt.asIntN(64, BigInt(1))]);
  }
  return h.hash([BigInt.asIntN(64, BigInt(0))]);
});
__publicField(_MtValue, "mkValueString", (h, v) => {
  return h.hashBytes(bytesEncoder.encode(v));
});
__publicField(_MtValue, "mkValueTime", async (h, v) => {
  return _MtValue.mkValueInt(h, v.epochNanoseconds);
});
__publicField(_MtValue, "mkValueBigInt", async (h, v) => {
  const prime = h.prime();
  if (v >= prime) {
...
var import_util = __toESM(require("jsonld/lib/util.js"), 1);
var import_constants4 = __toESM(require("jsonld/lib/constants.js"), 1);
var import_JsonLdError = __toESM(require("jsonld/lib/JsonLdError.js"), 1);
var import_url = __toESM(require("jsonld/lib/url.js"), 1);
var JsonLDLoader = class {
  async loadDocument(url, redirects = []) {
    const isHttp = url.startsWith("http:");
    const isHttps = url.startsWith("https:");
    if (!isHttp && !isHttps) {
      throw new import_JsonLdError.default(
    ...
    if (link && contentType !== "application/ld+json" && contentType !== "application/json") {
      const linkHeaders = import_util.default.parseLinkHeader(link);
      const linkedContext = linkHeaders[import_constants4.default.LINK_HEADER_CONTEXT];
      ...
      if (alternate && alternate["type"] == "application/ld+json" && !(contentType || "").match(/^application\/(\w*\+)?json$/)) {
        location = import_url.default.prependBase(url, alternate["target"]);
      }
    }
...
async function loadIPFS(url, ipfsNodeURL = void 0, ipfsGatewayURL = void 0) {
  ...
  if (!ipfsNodeURL && !ipfsGatewayURL) {
    throw new import_JsonLdError.default("IPFS is not configured", "jsonld.IPFSNotConfigured", {
      code: "loading document failed",
      url: documentURL
    });
  }
...
async function loadFromIPFSGateway(url, ipfsGatewayURL = void 0) {
  if (!ipfsGatewayURL) {
    throw new import_JsonLdError.default("IPFS gateway is not configured", "jsonld.IPFSNotConfigured", {
      code: "loading document failed",
...
// src/rdf-dataset.ts
var _RDFDataset = class _RDFDataset {
  constructor(graphs = /* @__PURE__ */ new Map()) {
    this.graphs = graphs;
  }
  static async fromDocument(doc, documentLoader = getDocumentLoader()) {
    const normalizedData = await import_jsonld.default.canonize(doc, {
      format: MerklizationConstants.QUADS_FORMAT,
      documentLoader
    });
    const parser = new import_n3.Parser({ format: MerklizationConstants.QUADS_FORMAT });
...
__publicField(_RDFDataset, "assertDatasetConsistency", (ds) => {
...
var _Path = class _Path {
  ...
  async pathFromContext(docStr, path, opts) {
    ...
    const emptyCtx = await import_jsonld2.default.processContext(null, null, jsonldOpts);
    let parsedCtx = await import_jsonld2.default.processContext(emptyCtx, doc, jsonldOpts);
    ...
    for (const i in parts) {
      ...
      if (MerklizationConstants.DIGITS_ONLY_REGEX.test(p)) {
        ...
        if (nextCtx) {
          parsedCtx = await import_jsonld2.default.processContext(parsedCtx, m, jsonldOpts);
        }
        this.parts.push(id);
      }
    }
  ...
  async typeFromContext(ctxStr, path, opts) {
    ...
    const jsonldOpts = { documentLoader: getDocumentLoader(opts) };
    const emptyCtx = await import_jsonld2.default.processContext(null, null, jsonldOpts);
    let parsedCtx = await import_jsonld2.default.processContext(emptyCtx, ctxObj, jsonldOpts);
    const parts = path.split(".");
    for (const i in parts) {
      const p = parts[i];
      const expP = expandType(parsedCtx, p);
      if (expP.hasContext) {
        parsedCtx = await import_jsonld2.default.processContext(parsedCtx, expP.typeDef, jsonldOpts);
      }
  ...
  static async pathFromDocument(ldCTX, doc, pathParts, acceptArray, opts) {
    ...
    if ("@context" in doc) {
      if (ldCTX) {
        ldCTX = await import_jsonld2.default.processContext(ldCTX, doc, jsonldOpts);
      } else {
        const emptyCtx = await import_jsonld2.default.processContext(null, null, jsonldOpts);
        ldCTX = await import_jsonld2.default.processContext(emptyCtx, doc, jsonldOpts);
      }
    }
    ...
    for (const k in elemKeys) {
      ...
      for (const tt of types) {
        ...
        if (typeof td === "object" && "@context" in td) {
          ldCTX = await import_jsonld2.default.processContext(ldCTX, td, jsonldOpts);
        }
      }
      break;
    }
    ...
    if (expTerm.hasContext) {
      if (ldCTX) {
        ldCTX = await import_jsonld2.default.processContext(ldCTX, expTerm.typeDef, jsonldOpts);
      } else {
        const emptyCtx = await import_jsonld2.default.processContext(null, null, jsonldOpts);
        ldCTX = await import_jsonld2.default.processContext(emptyCtx, expTerm.typeDef, jsonldOpts);
      }
    }
  ...
  static async getTypeIDFromContext(ctxStr, typeName, opts) {
    ...
    const jsonldOpts = { documentLoader: getDocumentLoader(opts) };
    const emptyCtx = await import_jsonld2.default.processContext(null, null, jsonldOpts);
    const parsedCtx = await import_jsonld2.default.processContext(emptyCtx, ctxObj, jsonldOpts);
...
__publicField(_Path, "newPath", (parts) => {
  const p = new _Path();
  p.append(parts);
  return p;
});
__publicField(_Path, "getContextPathKey", async (docStr, ctxTyp, fieldPath, opts) => {
  if (ctxTyp === "") {
...
// src/quad-arr-key.ts
var _QuadArrKey = class _QuadArrKey {
  constructor(q) {
...
__publicField(_QuadArrKey, "countEntries", (nodes) => {
...
__publicField(_RDFEntry, "newRDFEntry", (k, v) => {
  ...
  switch (typeof v) {
    ...
    case "boolean":
      ...
      break;
    default:
      if (v instanceof import_polyfill3.Temporal.Instant) {
        e.value = v;
      } else {
...
var Merklizer = class _Merklizer {
  constructor(srcDoc = null, mt = null, hasher = DEFAULT_HASHER, entries = /* @__PURE__ */ new Map(), compacted = null, documentLoader = getDocumentLoader()) {
    ...
    if (!mt) {
      ...
      this.mt = new import_js_merkletree2.Merkletree(db, writable, maxLevels);
    }
  }
  async proof(p) {
  ...
  static async merklizeJSONLD(docStr, opts) {
    ...
    mz.compacted = await import_jsonld3.default.compact(
      doc,
      {},
      {
        documentLoader,
