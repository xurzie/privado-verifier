...
var _Path = class _Path {
  ...
  async pathFromContext(docStr, path, opts) {
    ...
    const emptyCtx = await jsonld2.processContext(null, null, jsonldOpts);
    let parsedCtx = await jsonld2.processContext(emptyCtx, doc, jsonldOpts);
    ...
    for (const i in parts) {
      ...
      if (MerklizationConstants.DIGITS_ONLY_REGEX.test(p)) {
        ...
        if (nextCtx) {
          parsedCtx = await jsonld2.processContext(parsedCtx, m, jsonldOpts);
        }
        this.parts.push(id);
      }
    }
  ...
  async typeFromContext(ctxStr, path, opts) {
    ...
    const jsonldOpts = { documentLoader: getDocumentLoader(opts) };
    const emptyCtx = await jsonld2.processContext(null, null, jsonldOpts);
    let parsedCtx = await jsonld2.processContext(emptyCtx, ctxObj, jsonldOpts);
    const parts = path.split(".");
    for (const i in parts) {
      const p = parts[i];
      const expP = expandType(parsedCtx, p);
      if (expP.hasContext) {
        parsedCtx = await jsonld2.processContext(parsedCtx, expP.typeDef, jsonldOpts);
      }
  ...
  static async pathFromDocument(ldCTX, doc, pathParts, acceptArray, opts) {
    ...
    if ("@context" in doc) {
      if (ldCTX) {
        ldCTX = await jsonld2.processContext(ldCTX, doc, jsonldOpts);
      } else {
        const emptyCtx = await jsonld2.processContext(null, null, jsonldOpts);
        ldCTX = await jsonld2.processContext(emptyCtx, doc, jsonldOpts);
      }
    }
    ...
    for (const k in elemKeys) {
      ...
      for (const tt of types) {
        ...
        if (typeof td === "object" && "@context" in td) {
          ldCTX = await jsonld2.processContext(ldCTX, td, jsonldOpts);
        }
      }
      break;
    }
    ...
    if (expTerm.hasContext) {
      if (ldCTX) {
        ldCTX = await jsonld2.processContext(ldCTX, expTerm.typeDef, jsonldOpts);
      } else {
        const emptyCtx = await jsonld2.processContext(null, null, jsonldOpts);
        ldCTX = await jsonld2.processContext(emptyCtx, expTerm.typeDef, jsonldOpts);
      }
    }
  ...
  static async getTypeIDFromContext(ctxStr, typeName, opts) {
    ...
    const jsonldOpts = { documentLoader: getDocumentLoader(opts) };
    const emptyCtx = await jsonld2.processContext(null, null, jsonldOpts);
    const parsedCtx = await jsonld2.processContext(emptyCtx, ctxObj, jsonldOpts);
...
var Merklizer = class _Merklizer {
  ...
  static async merklizeJSONLD(docStr, opts) {
    ...
    mz.compacted = await jsonld3.compact(
      doc,
      {},
      {
        documentLoader,
