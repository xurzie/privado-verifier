...
class ProofService {
    ...
    async generateAuthProof(circuitId, identifier, opts) {
        ...
        let zkProof;
        switch (circuitId) {
            case circuits_1.CircuitId.AuthV2:
                {
                    const challenge = opts.challenge
                        ? js_iden3_core_1.BytesHelper.intToBytes(opts.challenge).reverse()
                        : new Uint8Array(32);
    ...
    async generateAuthV2Proof(challenge, did) {
        const authInputs = await this.generateAuthV2Inputs(challenge, did, circuits_1.CircuitId.AuthV2);
        const zkProof = await this._prover.generate(authInputs, circuits_1.CircuitId.AuthV2);
        return zkProof;
    }
