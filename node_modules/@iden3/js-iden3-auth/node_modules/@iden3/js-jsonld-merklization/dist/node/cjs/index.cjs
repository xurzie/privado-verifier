...
var import_jsonld3 = require("jsonld");
...
var import_util = require("jsonld/lib/util");
var import_constants4 = require("jsonld/lib/constants");
var import_JsonLdError = __toESM(require("jsonld/lib/JsonLdError"), 1);
var import_url = require("jsonld/lib/url");
var JsonLDLoader = class {
  async loadDocument(url, redirects = []) {
    ...
    if (link && contentType !== "application/ld+json" && contentType !== "application/json") {
      const linkHeaders = (0, import_util.parseLinkHeader)(link);
      const linkedContext = linkHeaders[import_constants4.LINK_HEADER_CONTEXT];
      ...
      if (alternate && alternate["type"] == "application/ld+json" && !(contentType || "").match(/^application\/(\w*\+)?json$/)) {
        location = (0, import_url.prependBase)(url, alternate["target"]);
      }
    }
...
var RDFDataset = class _RDFDataset {
  ...
  static async fromDocument(doc, documentLoader = getDocumentLoader()) {
    const normalizedData = await (0, import_jsonld.canonize)(doc, {
...
// src/lib/path.ts
var import_jsonld2 = require("jsonld");
var Path = class _Path {
  ...
  async pathFromContext(docStr, path, opts) {
    ...
    const emptyCtx = await (0, import_jsonld2.processContext)(null, null, jsonldOpts);
    let parsedCtx = await (0, import_jsonld2.processContext)(emptyCtx, doc, jsonldOpts);
    ...
    for (const i in parts) {
      ...
      if (MerklizationConstants.DIGITS_ONLY_REGEX.test(p)) {
        ...
        if (nextCtx) {
          parsedCtx = await (0, import_jsonld2.processContext)(parsedCtx, m, jsonldOpts);
        }
        this.parts.push(id);
      }
    }
  ...
  async typeFromContext(ctxStr, path, opts) {
    ...
    const jsonldOpts = { documentLoader: getDocumentLoader(opts) };
    const emptyCtx = await (0, import_jsonld2.processContext)(null, null, jsonldOpts);
    let parsedCtx = await (0, import_jsonld2.processContext)(emptyCtx, ctxObj, jsonldOpts);
    const parts = path.split(".");
    for (const i in parts) {
      const p = parts[i];
      const expP = expandType(parsedCtx, p);
      if (expP.hasContext) {
        parsedCtx = await (0, import_jsonld2.processContext)(parsedCtx, expP.typeDef, jsonldOpts);
      }
  ...
  static async pathFromDocument(ldCTX, doc, pathParts, acceptArray, opts) {
    ...
    if ("@context" in doc) {
      if (ldCTX) {
        ldCTX = await (0, import_jsonld2.processContext)(ldCTX, doc, jsonldOpts);
      } else {
        const emptyCtx = await (0, import_jsonld2.processContext)(null, null, jsonldOpts);
        ldCTX = await (0, import_jsonld2.processContext)(emptyCtx, doc, jsonldOpts);
      }
    }
    ...
    for (const k in elemKeys) {
      ...
      for (const tt of types) {
        ...
        if (typeof td === "object" && "@context" in td) {
          ldCTX = await (0, import_jsonld2.processContext)(ldCTX, td, jsonldOpts);
        }
      }
      break;
    }
    ...
    if (expTerm.hasContext) {
      if (ldCTX) {
        ldCTX = await (0, import_jsonld2.processContext)(ldCTX, expTerm.typeDef, jsonldOpts);
      } else {
        const emptyCtx = await (0, import_jsonld2.processContext)(null, null, jsonldOpts);
        ldCTX = await (0, import_jsonld2.processContext)(emptyCtx, expTerm.typeDef, jsonldOpts);
      }
    }
  ...
  static async getTypeIDFromContext(ctxStr, typeName, opts) {
    ...
    const jsonldOpts = { documentLoader: getDocumentLoader(opts) };
    const emptyCtx = await (0, import_jsonld2.processContext)(null, null, jsonldOpts);
    const parsedCtx = await (0, import_jsonld2.processContext)(emptyCtx, ctxObj, jsonldOpts);
...
var Merklizer = class _Merklizer {
  ...
  static async merklizeJSONLD(docStr, opts) {
    ...
    mz.compacted = await (0, import_jsonld3.compact)(
      doc,
      {},
      {
        documentLoader,
